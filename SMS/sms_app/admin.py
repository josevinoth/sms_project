from django.contrib import admin
from .models import DeletionLog, TransitdistributeInfo,damage_image_type_info,ExcessStock,hptforkliftcraneInfo,storagecrosslabelInfo, VehicletypeInfo,task_Info,applicaiton_Info,assign_status_info,pk_stock_statusinfo,pk_itemdescriptionInfo,pk_itemInfo,Pkstockpurchasetype,ExpenseCategoryInfo,SealedoropenedInfo,TypeofotlInfo,Stockdescription,Places,Currency_type,Bunkname,Fuelvendor,Pkstocktype,Trip_category_info,Unitofmeasure,Costdescription,Costtype,GST_payable_info,Tr_businesstype_Info,Tr_triptype_Info,Source,Category,Stock_type,Naconsumables,Natypeofaccess,Nawoodnorms,Nabvmcustomer,Natypeofwood,Nawoodtreatmentreq,Natypeofplywood,Naplywoodthickness,Natypeofreq,Natypeofpack,Natypeofwork,Naitemname,Reqbugimprove,Reqmodule,Tripstatusinfo,Labels_pasted_Info,Bvmproduct,Business_Sol_info,Prespectivec_customer_NoInfo,Business_won_NoInfo,YesNoInfo,faciltiyrequirementinfo,manpowerrequirementinfo,CustomerInfo,CustomertypeInfo,Busnotwon,Salestatus,Callpurpose,Callnature,Calltype,Supplyinfo,Packreuqirementinfo,Transrequirementinfo,Whrequirementinfo,Industrytype,Cusnewexist,servicetype_info,ExpenseTypeInfo,ExpenseUOMInfo,Fumigation_ActionInfo,Department_info,RoleInfo,PaymentcycleInfo,StatusList,Country,State,City,Check_in_out,GstexcemptionInfo,UOM,Received_not,UploadInfo,DamagereportInfo,ActiveinactiveInfo,StackingInfo,Wh_chargetype,Nadeliverytype,Napackingfield,Naspecialrequirements,Nadimensiontype, PkstockpurchaseStatus ,Enquiry_source,Quote_status,Transport_mode,Vendor_description

# Register your models here.
admin.site.register(damage_image_type_info)
admin.site.register(task_Info)
admin.site.register(applicaiton_Info)
admin.site.register(assign_status_info)
admin.site.register(StatusList)
admin.site.register(DeletionLog)
admin.site.register(Country)
admin.site.register(State)
admin.site.register(City)
admin.site.register(RoleInfo)
admin.site.register(Department_info)
admin.site.register(Check_in_out)
admin.site.register(GstexcemptionInfo)
admin.site.register(UOM)
admin.site.register(Received_not)
admin.site.register(UploadInfo)
admin.site.register(DamagereportInfo)
admin.site.register(ActiveinactiveInfo)
admin.site.register(StackingInfo)
admin.site.register(Wh_chargetype)
admin.site.register(PaymentcycleInfo)
admin.site.register(Fumigation_ActionInfo)
admin.site.register(ExpenseTypeInfo)
admin.site.register(ExpenseUOMInfo)
admin.site.register(servicetype_info)
admin.site.register(Cusnewexist)
admin.site.register(Industrytype)
admin.site.register(Whrequirementinfo)
admin.site.register(Transrequirementinfo)
admin.site.register(Packreuqirementinfo)
admin.site.register(Supplyinfo)
admin.site.register(Calltype)
admin.site.register(Callnature)
admin.site.register(Callpurpose)
admin.site.register(Salestatus)
admin.site.register(Busnotwon)
admin.site.register(CustomertypeInfo)
admin.site.register(CustomerInfo)
admin.site.register(manpowerrequirementinfo)
admin.site.register(faciltiyrequirementinfo)
admin.site.register(YesNoInfo)
admin.site.register(Business_won_NoInfo)
admin.site.register(Prespectivec_customer_NoInfo)
admin.site.register(Business_Sol_info)
admin.site.register(Bvmproduct)
admin.site.register(Labels_pasted_Info)
admin.site.register(Tripstatusinfo)
admin.site.register(Reqmodule)
admin.site.register(Reqbugimprove)
admin.site.register(Naitemname)
admin.site.register(Natypeofwork)
admin.site.register(Natypeofpack)
admin.site.register(Natypeofreq)
admin.site.register(Naplywoodthickness)
admin.site.register(Natypeofplywood)
admin.site.register(Nawoodtreatmentreq)
admin.site.register(Natypeofwood)
admin.site.register(Nabvmcustomer)
admin.site.register(Nawoodnorms)
admin.site.register(Natypeofaccess)
admin.site.register(Naconsumables)
admin.site.register(Stock_type)
admin.site.register(Category)
admin.site.register(Source)
admin.site.register(Tr_businesstype_Info)
admin.site.register(Tr_triptype_Info)
admin.site.register(GST_payable_info)
admin.site.register(Costtype)
admin.site.register(Costdescription)
admin.site.register(Unitofmeasure)
admin.site.register(Trip_category_info)
admin.site.register(Pkstocktype)
admin.site.register(Fuelvendor)
admin.site.register(Bunkname)
admin.site.register(Currency_type)
admin.site.register(Places)
admin.site.register(Stockdescription)
admin.site.register(TypeofotlInfo)
admin.site.register(SealedoropenedInfo)
admin.site.register(ExpenseCategoryInfo)
admin.site.register(Pkstockpurchasetype)
admin.site.register(pk_itemInfo)
admin.site.register(pk_itemdescriptionInfo)
admin.site.register(pk_stock_statusinfo)
admin.site.register(VehicletypeInfo)
admin.site.register(Nadeliverytype)
admin.site.register(Nadimensiontype)
admin.site.register(Napackingfield)
admin.site.register(Naspecialrequirements)
admin.site.register(PkstockpurchaseStatus)
admin.site.register(ExcessStock)
admin.site.register(storagecrosslabelInfo)
admin.site.register(hptforkliftcraneInfo)
admin.site.register(TransitdistributeInfo)
admin.site.register(Quote_status)
admin.site.register(Transport_mode)
admin.site.register(Enquiry_source)
admin.site.register(Vendor_description)