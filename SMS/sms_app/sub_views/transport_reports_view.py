...
def vendor_p_l_mkt_report_ajax_view(request):
    ...
    trips = TripdetailInfo.objects.filter(
        ...
        Q(tr_category_id=1, tr_departeddate__isnull=False) |
...
def own_vehicle_pl_report_view(request):
    ...
    if is_ajax:
        ...
        trips = TripdetailInfo.objects.filter(
            ...
            Q(tr_category_id=1, tr_departeddate__isnull=False, tc_financestatus_id__in=[2, 7, 9]) |
            Q(tr_category_id__in=[2, 3])
        ).select_related(
            'tr_vehicletype',
            'tr_category',
...
def get_filtered_trips(branch_id, trip_category_id, vehicle_source_id, from_date, to_date, selected_year,
    ...
    trips = trips.filter(
        tr_departeddate__isnull=False
    ).annotate(
        resolved_date=F('tr_departeddate')
    )
