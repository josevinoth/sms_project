...
def resolve_trip_allotment(trip, allotment_map=None):
    if not trip or not trip.tr_enquirynumber_id:
        return None
    if allotment_map is not None:
        if isinstance(allotment_map, dict):
            clean_veh = str(trip.tr_vehiclenumber or '').replace(" ", "").replace("-", "").upper()
            if (trip.tr_enquirynumber_id, clean_veh) in allotment_map:
                return allotment_map[(trip.tr_enquirynumber_id, clean_veh)]
            if (trip.tr_enquirynumber_id, trip.tr_vehiclenumber) in allotment_map:
                return allotment_map[(trip.tr_enquirynumber_id, trip.tr_vehiclenumber)]
            if trip.tr_enquirynumber_id in allotment_map:
                val = allotment_map[trip.tr_enquirynumber_id]
                if isinstance(val, list):
                    if clean_veh:
                        for a in val:
                            a_veh = ""
                            if a.va_vehiclenumber:
                                a_veh = str(getattr(a.va_vehiclenumber, 'vm_registrationnumber', '') or '')
                            elif a.va_vehiclenumber_mkt:
                                a_veh = str(a.va_vehiclenumber_mkt or '')
                            if a_veh.replace(" ", "").replace("-", "").upper() == clean_veh:
                                return a
    ...
    allotments = Vehicle_allotmentInfo.objects.filter(
        va_enquirynumber_id=trip.tr_enquirynumber_id
    ).select_related('va_vehiclenumber', 'va_vehicletype', 'va_vehicletype_placed')
    if not allotments.exists():
        return None
    if trip.tr_vehiclenumber:
        clean_veh = str(trip.tr_vehiclenumber).replace(" ", "").replace("-", "").upper()
        for a in allotments:
            a_veh = ""
            if a.va_vehiclenumber:
                a_veh = str(getattr(a.va_vehiclenumber, 'vm_registrationnumber', '') or '')
...
def get_tms_report_transport_charge(trip, inv=None, allotment=None, allotment_map=None):
    """
    ...
    is_settled = getattr(trip, 'tc_financestatus_id', None) in [7, 9]
...
def calculate_own_vehicle_fuel_and_salary(trips_list, date_from=None, date_to=None):
    ...
    if distinct_months:
        ...
        for mt in all_trips_in_months:
            ...
            if not mt_date:
                continue
            if timezone.is_aware(mt_date):
                mt_date = timezone.localtime(mt_date)
    ...
    for t_id, d_id, m, y in trip_driver_info:
        salary_lookup[t_id] = salary_lookup_raw.get((d_id, m, y), 0.0)

    # ==========================
    # 2. OPTIMIZED FUEL CALC
    # ==========================
    def _trip_report_date(trip):
        ...
        if d and timezone.is_aware(d):
            d = timezone.localtime(d)
        return d.date() if hasattr(d, 'date') else d

    # Collect all own vehicles involved
    veh_nos = set()
    trips_by_veh = {}
    ...
    for t in trips_list:
        ...
        if veh_no:
            ...
            if trip_date:
                trips_by_veh.setdefault(veh_no, []).append((t, trip_date))

    # Fetch all fuel fills for these vehicles, sorted
    all_fuel = Fuelfillinginfo.objects.filter(
        ff_vehicle_num__vm_registrationnumber__in=list(veh_nos)
    ).select_related('ff_vehicle_num').order_by('ff_vehicle_num__vm_registrationnumber', 'ff_date', 'ff_odometer_reading', 'id')
    ...
    if veh_nos:
        ...
        all_period_trips = TripdetailInfo.objects.filter(
            tr_vehiclenumber__in=list(veh_nos),
            tr_category_id__in=[1, 2, 3]
        ).annotate(
        ...
        for pt in all_period_trips:
            vno = pt['tr_vehiclenumber'].strip()
            # inline vehicle_log_used_km logic
            start_km = safe_num(pt['tr_reportedkm_pickup']) if pt['tr_reportedkm_pickup'] else safe_num(pt['tr_departedkm'])
            closing_km = safe_num(pt['tr_reportedkm_delivery']) if pt['tr_reportedkm_delivery'] else safe_num(pt['tr_reportedkm'])
            used_km = 0
            if closing_km and start_km:
                ukm = closing_km - start_km
                if 0 < ukm < 15000:
                    used_km = ukm
            if used_km > 0:
                rd = pt['resolved_date']
                if rd:
                    ...
                    all_trips_by_veh.setdefault(vno, []).append((rd_date, used_km))
                
    for veh_no, t_list in trips_by_veh.items():
        v_fuel = fuel_by_veh.get(veh_no, [])
        if not v_fuel:
            for t, td in t_list: fuel_lookup[t.id] = 0.0
            continue
            
        v_all_trips = all_trips_by_veh.get(veh_no, [])
        
        # build intervals
        intervals = []
        for idx, f_rec in enumerate(v_fuel):
            interval_start = f_rec.ff_date
            next_rec = v_fuel[idx + 1] if idx + 1 < len(v_fuel) else None
            interval_end = next_rec.ff_date if next_rec else None
            include_end = next_rec is None
            
            # calculate total km in this interval
            total_km = 0
            for pt_date, ukm in v_all_trips:
                if pt_date >= interval_start:
                    if interval_end is None or (pt_date <= interval_end if include_end else pt_date < interval_end):
                        total_km += ukm
            
            f_price = f_rec.ff_fuel_price
            f_price = float(f_price) if f_price else 0.0
            per_km = (f_price / total_km) if total_km > 0 else 0.0
            intervals.append((interval_start, interval_end, include_end, per_km))
            
        for t, trip_date in t_list:
            used_km = 0
            start_km = safe_num(t.tr_reportedkm_pickup) if t.tr_reportedkm_pickup else safe_num(t.tr_departedkm)
            closing_km = safe_num(t.tr_reportedkm_delivery) if t.tr_reportedkm_delivery else safe_num(t.tr_reportedkm)
            if closing_km and start_km:
                ukm = closing_km - start_km
                if 0 < ukm < 15000:
                    used_km = ukm
            ...
            trip_fuel_cost = 0.0
            if used_km > 0:
                for istart, iend, inc_end, per_km in intervals:
                    if trip_date >= istart:
                        if iend is None or (trip_date <= iend if inc_end else trip_date < iend):
                            trip_fuel_cost = used_km * per_km
                            break
...
def trip_cancellation_report_ajax_view(request):
    ...
    for idx, trip in enumerate(page_trips, start=start + 1):
        ...
        rate_sheet_charge = 0
        try:
            cid = trip.tr_enquirynumber.en_customername_id if trip.tr_enquirynumber else None
            vid = trip.tr_vehicletype_placed_id or (trip.tr_vehicletype_id if trip.tr_vehicletype else None)
            if cid and vid:
                cms = ChargeMasterInfo.objects.filter(
                    cm_customer_id=cid,
                    cm_vehicle_type_id=vid,
                    cm_charge_type_id=3  # 3 = Cancellation
                )
                for c in cms:
                    if c.cm_charge_type_id == 3:
                        rate_sheet_charge = c.cm_amount
        except Exception:
            pass
        ...
        data_rows.append([
            ...
            _fmt(trip.tr_departeddate),
            _fmt(trip.tr_reporteddate),
            safe_str(trip.tr_departedlocation),
            safe_str(trip.tr_reportedlocation),
            safe_str(trip.tr_vehiclenumber),
...
def vehicle_utilization_report_view(request):
    ...
    for vehicle in vehicles:
        ...
        if month_start.year == today_dt.year and month_start.month == today_dt.month:
    ...
    context = {
        ...
        'data_rows': data_rows,
        'selected_month': str(month_start.month) if month_start else '0',
        'selected_year': str(month_start.year) if month_start else '0',
        'all_vehicles': all_vehicles,
        'vehicle_search': vehicle_search,
...
def ref_no_pending_report_ajax_view(request):
    ...
    # Needs _fmt_dt function, we'll recreate or call it if it exists. 
    ...
    for idx, trip in enumerate(page_trips, start=start + 1):
        ...
        try:
            ...
            if cid and vid:
                cms = ChargeMasterInfo.objects.filter(
                    cm_customer_id=cid,
                    cm_vehicle_type_id=vid,
                    cm_charge_type_id__in=[1, 2]
                )
                for c in cms:
                    if c.cm_charge_type_id == 1: trip_charge = c.cm_amount
                    elif c.cm_charge_type_id == 2: toll_charge = c.cm_amount
        except Exception:
            pass
        ...
        transport_val = get_tms_report_transport_charge(trip)
        total_selling = safe_num(transport_val) + \
        ...
        data_rows.append([
            ...
            halting_val,
            safe_num(trip.tc_handlingcost) if trip.tc_handlingcost_check else 0,
            round(safe_num(total_selling), 2)
        ])
...
def invoice_pending_report_ajax_view(request):
    ...
    for idx, trip in enumerate(trips_slice, start=start + 1):
        ...
        transport_val = get_tms_report_transport_charge(trip, allotment=allotment)
...
def vendor_p_l_mkt_report_ajax_view(request):
    ...
    trips = TripdetailInfo.objects.filter(
        ...
        resolved_date=Case(
            When(tr_category_id=1, then=F('tr_departeddate')),
            default=Coalesce(
    ...
    for idx, trip in enumerate(trips_list, start=1):
            ...
            for a in allotments:
                if a.va_enquirynumber_id == trip.tr_enquirynumber_id:
                    ...
                    if a_veh_no.replace(" ", "").upper() == clean_veh:
                        allotment = a
                        break
        if not allotment and trip.tr_enquirynumber_id:
            for a in allotments:
                if a.va_enquirynumber_id == trip.tr_enquirynumber_id:
                    allotment = a
                    break
        ...
        selling_aai = safe_num(inv.ti_docket_charges) if inv else (safe_num(trip.tc_supervisorcost) if getattr(trip, 'tc_supervisorcost_check', True) else 0.0)
...
def vendor_p_l_attached_report_ajax_view(request):
    ...
    if search_value:
        ...
        trips_listing = trips_listing.filter(q_objects)
        trips = trips.filter(q_objects)

    order_dir = request.GET.get('order[0][dir]', 'desc')
    order_field = '-tr_created_at' if order_dir == 'desc' else 'tr_created_at'
    trips_listing = trips_listing.order_by(order_field)
    trips = trips.order_by(order_field)

    all_trips = list(trips)
    all_trips_listing = list(trips_listing)
    records_filtered = len(all_trips_listing)

    vehicle_trip_counts = {}
    for t in all_trips:
        if t.tr_vehiclenumber:
            clean_no = t.tr_vehiclenumber.replace(" ", "").upper()
            vehicle_trip_counts[clean_no] = vehicle_trip_counts.get(clean_no, 0) + 1

    if length != -1:
        trips_slice = all_trips_listing[start:start + length]
    else:
        trips_slice = all_trips_listing

    trips_list = list(trips_slice)
    trip_ids = [t.id for t in trips_list]
    trip_enquiries = [t.tr_enquirynumber_id for t in trips_list]

    # Map collection ids for ALL trips
    all_trip_ids = [t.id for t in all_trips]
    ...
    cnote_to_pk = {str(t.tr_consignmentnumber.co_consignmentnumber).strip(): t.id for t in all_trips if t.tr_consignmentnumber and t.tr_consignmentnumber.co_consignmentnumber}

    all_query_ids = [str(t.id) for t in all_trips]
    for t in all_trips:
    ...
    for idx, trip in enumerate(trips_list, start=1):
        ...
        if trip.tr_category_id in [2, 3]:
            selling_trip = selling_toll = selling_aai = selling_loading = selling_unloading = 0.0
        ...
        if att_bill:
            ...
            if veh_master:
                vm_buycost_val = safe_num(veh_master.vm_buycost)
                trip_count = vehicle_trip_counts.get(clean_veh_no, 0)
                if trip_count > 0:
    ...
    # SUMMARY AGGREGATION
    # -------------------------------
    total_trips_sum = sum(1 for t in all_trips if t.tr_category_id not in [2, 3])
    ...
    total_buy_cost = 0.0
    total_toll_cost = 0.0
    total_buying_expenses = 0.0

    # Calculate total calendar days in the selected period (inclusive)
    total_days_in_period = 30  # default: 1 month
    if date_from and date_to:
        try:
            d_from = datetime.strptime(date_from, "%Y-%m-%d")
            d_to = datetime.strptime(date_to, "%Y-%m-%d")
            total_days_in_period = max(1, (d_to - d_from).days + 1)
            months_count = max(1.0, total_days_in_period / 30.0)
        except:
            months_count = 1
    ...
    for trip in all_trips:
        if trip.resolved_date:
            date_val = trip.resolved_date
            if isinstance(date_val, datetime):
                if timezone.is_aware(date_val):
                    date_val = timezone.localtime(date_val)
                working_days_set.add(date_val.date())
            elif isinstance(date_val, date):
                working_days_set.add(date_val)
        ...
        if att_bill:
            total_trips_ab = len([t for t in str(att_bill.ab_selected_trips).split(',') if t.strip()])
            bill_amt = safe_num(att_bill.ab_bill_amount)
            total_km_ab = safe_num(att_bill.ab_total_km_run)
            
            if total_km_ab > 0 and bill_amt > 0:
                trip_km_ab = 0
                start_km_ab = trip.tr_reportedkm_pickup if trip.tr_reportedkm_pickup else (trip.tr_departedkm or 0)
                closing_km_ab = trip.tr_reportedkm_delivery if trip.tr_reportedkm_delivery else (trip.tr_reportedkm or 0)
                if closing_km_ab and start_km_ab:
                    diff_ab = closing_km_ab - start_km_ab
                    if 0 < diff_ab < 15000:
                        trip_km_ab = diff_ab
                cost_per_km = bill_amt / total_km_ab
                buying_trip_cost = round(trip_km_ab * cost_per_km, 2)
            elif total_trips_ab > 0:
                buying_trip_cost = round(safe_num(att_bill.ab_buy_cost) / total_trips_ab, 2)
        else:
            if veh_master:
                vm_buycost_val = safe_num(veh_master.vm_buycost)
                trip_count = vehicle_trip_counts.get(clean_veh_no, 0)
        ...
        total_buy_cost += buying_trip_cost
        total_toll_cost += buying_toll
        total_buying_expenses += total_buying

    num_days_working = len(working_days_set)
    num_idle_days = max(0, total_days_in_period - num_days_working)
    if not veh_no and total_trips_sum > 0:
        unique_veh_count = len(set(t.tr_vehiclenumber for t in all_trips if t.tr_vehiclenumber))
        num_idle_days = max(0, (total_days_in_period * unique_veh_count) - num_days_working)

    empty_km_cost = 0.0
    business_empty_km_cost = 0.0
    if total_km > 0:
        cost_per_km = total_buying_expenses / total_km
        empty_km_cost = empty_km_val * cost_per_km
        business_empty_km_cost = business_empty_km_val * cost_per_km

    total_expenses_all = total_buying_expenses + empty_km_cost + business_empty_km_cost
    ...
    summary_vehicle_no = veh_no if veh_no else "All Vehicles"
    summary_vehicle_type = "Mixed"
    if vehicle_type_id:
        try:
            vt = VehicletypeInfo.objects.get(id=vehicle_type_id)
            summary_vehicle_type = vt.vt_vehicletype
        except:
            pass
    elif veh_no and all_trips:
        first_t = all_trips[0]
        summary_vehicle_type = str(first_t.tr_vehicletype_placed or first_t.tr_vehicletype or "Mixed")
    ...
    summary_data = {
        ...
        'total_trips': total_trips_sum,
        'trip_index': round(total_trips_sum / max(1, num_days_working), 2),
...
def get_pl_vehicles_by_vendor(request):
    ...
    # Also check Market Vehicles which are stored directly as strings
    va_qs_mkt = Vehicle_allotmentInfo.objects.filter(
        va_vendor_id=vendor_id,
        va_vehiclenumber_mkt__isnull=False
    ).values_list(
        'va_vehiclenumber_mkt', flat=True
    ).distinct()
...
def whatsapp_delivery_status_report_ajax_view(request):
    ...
    if length == -1:
        page_trips = trips[start:]
    else:
        page_trips = trips[start:start + length]

    data_rows = []
    for idx, trip in enumerate(page_trips, start=start + 1):
        customer_name = safe_str(trip.tr_enquirynumber.en_customername) if trip.tr_enquirynumber else ""
        department = safe_str(trip.tr_enquirynumber.en_customerdepartment) if trip.tr_enquirynumber else ""
        cnote_no = safe_str(trip.tr_consignmentnumber.co_consignmentnumber) if trip.tr_consignmentnumber else ""
...
def own_vehicle_pl_report_view(request):
    ...
    if is_ajax:
        ...
        trips = TripdetailInfo.objects.filter(
            ...
            Q(tr_category_id=1, tr_departeddate__isnull=False, tc_financestatus_id__in=[2, 4, 5, 6, 7, 9]) |
        ...
        # Collect all possible search strings — only actual trip number strings, NOT raw DB IDs
        # Using raw DB IDs causes expense records to be matched to wrong trips (false positives)
        all_query_ids = [
            str(t.tr_tripnumber).strip() for t in trips_list if t.tr_tripnumber and str(t.tr_tripnumber).strip()
        ] + [
            str(t.tr_tripnumber).strip().upper() for t in trips_list if t.tr_tripnumber and str(t.tr_tripnumber).strip()
        ]
        ...
        for trip in trips_list:
            ...
            if trip.tr_category_id in [2, 3]:
                ...
                selling_trip = get_tms_report_transport_charge(trip, inv=inv)
            ...
            # Accumulate summary metrics inside the loop to avoid array zip mismatch issues
            if trip.tr_departeddate:
                dep_date = trip.tr_departeddate
                if timezone.is_aware(dep_date):
                    dep_date = timezone.localtime(dep_date)
                working_days_set.add(dep_date.date())
            ...
            if trip.tr_category_id not in [2, 3]:
                total_revenue += selling_total
                total_operational_expense += total_expense

            # Include toll cost for ALL trips (including Cat 2 and 3)
            total_toll_cost += toll_expense
        ...
        # SUMMARY AGGREGATION
        # -------------------------------
        total_trips = sum(1 for trip in trips_list if trip.tr_category_id not in [2, 3])
        ...
        if date_from and date_to:
            try:
                d_from = datetime.strptime(date_from, "%Y-%m-%d")
                d_to = datetime.strptime(date_to, "%Y-%m-%d")
                # +1 makes it inclusive: June 1 to June 30 = 30 days
                total_days_in_period = max(1, (d_to - d_from).days + 1)
                months_count = max(1.0, total_days_in_period / 30.0)
            except:
                months_count = 1
...
def diesel_vs_revenue_report_ajax_view(request):
    ...
    if length == -1:
        ...
        page_trip_ids = trip_ids[start:start + length]
        
    page_trips = TripdetailInfo.objects.filter(id__in=page_trip_ids).select_related(
        'tr_enquirynumber',
        'tr_enquirynumber__en_customername',
        'tr_consignmentnumber',
        'tr_vehicletype',
    ...
    # To be accurate with vehicle averages, we should calculate for the filtered vehicle set.
    ...
    if from_date and to_date:
        fuel_q &= Q(ff_date__gte=from_date, ff_date__lte=to_date)
    ...
    fleet_avg_price = (total_fleet_cost / total_fleet_ltr) if total_fleet_ltr > 0 else 96.0
    
    page_vehicle_numbers = page_trips.values_list('tr_vehiclenumber', flat=True).distinct()
    vehicles = VehiclemasterInfo.objects.filter(vm_registrationnumber__in=page_vehicle_numbers).select_related(
        'vm_vehiclemanufacturer', 'vm_vehiclemodel')
    vehicle_master_map = {v.vm_registrationnumber: v for v in vehicles}
    
    invoices = TransInvoiceInfo.objects.filter(ti_trip_id__in=[t.id for t in page_trips])
    invoice_map = {inv.ti_trip_id: inv for inv in invoices}

    enquiry_ids = [t.tr_enquirynumber_id for t in page_trips if t.tr_enquirynumber_id]
    ...
    for idx, trip in enumerate(page_trips, start=start + 1):
        ...
        rev_trip = get_tms_report_transport_charge(trip, inv=inv)
        if rev_trip == 0 and not inv:
...
def own_vs_market_sales_report_view(request):
    ...
    for trip in trips:
        ...
        if not full_date:
            continue
        if timezone.is_aware(full_date):
            full_date = timezone.localtime(full_date)
        ...
        transport_val = get_tms_report_transport_charge(trip, allotment=va_map.get(trip.tr_enquirynumber_id))
        revenue = safe_num(transport_val) + \
                  (safe_num(trip.tc_rtocost) if trip.tc_rtocost_check else 0) + \
                  (safe_num(trip.tc_betacost) if trip.tc_betacost_check else 0) + \
                  (safe_num(trip.tc_parkingcost) if trip.tc_parkingcost_check else 0) + \
                  (safe_num(trip.tc_tollcost) if trip.tc_tollcost_check else 0) + \
...
def get_filtered_trips(branch_id, trip_category_id, vehicle_source_id, from_date, to_date, selected_year,
    ...
    ).exclude(tr_category_id__in=[2, 3])
...
def location_pl_report_view(request):
    ...
    trips_list = [t for t in trips if t.tr_category_id not in [2, 3]]
    trip_ids = [t.id for t in trips_list]
    trip_enquiries = [t.tr_enquirynumber_id for t in trips_list]
    ...
    # -------------------------------
    # PRE-CALCULATE PRORATED FUEL COST AND SALARY PER TRIP
    # -------------------------------
    fuel_lookup, salary_lookup = calculate_own_vehicle_fuel_and_salary(
        trips_list, date_from=date_from, date_to=date_to
    )
    ...
    for trip in trips_list:
        ...
        if v_source == 1:
            selling_trip = get_tms_report_transport_charge(trip, inv=inv, allotment=va_map.get(trip.tr_enquirynumber_id))
            selling_toll = (safe_num(trip.tc_tollcost) if (
                    getattr(trip, 'tc_tollcost_check', True) and safe_num(trip.tc_tollcost) > 0) else (
                safe_num(inv.ti_toll_charges) if inv else 0.0))
            selling_parking = (safe_num(trip.tc_parkingcost) if (
            ...
            selling_supervisor = 0.0  # Exclude supervisor cost for own vehicles
            selling_rto = (safe_num(trip.tc_rtocost) if getattr(trip, 'tc_rtocost_check', True) else 0.0)
            selling_beta = (safe_num(trip.tc_betacost) if getattr(trip, 'tc_betacost_check', True) else 0.0)
            selling_cancellation = (safe_num(trip.tc_cancellation) if (
                    getattr(trip, 'tc_cancellation_check', True) and safe_num(trip.tc_cancellation) > 0) else (
            ...
            if trip.tr_category_id in [2, 3]:
                selling_trip = selling_toll = selling_supervisor = selling_loading = selling_unloading = 0.0
                selling_weighment = selling_halting = selling_handling = selling_parking = 0.0
                selling_rto = selling_beta = selling_cancellation = 0.0
            else:
                selling_trip = get_tms_report_transport_charge(trip, inv=inv, allotment=va_map.get(trip.tr_enquirynumber_id))
                ...
                selling_supervisor = safe_num(inv.ti_docket_charges) if inv else (safe_num(trip.tc_supervisorcost) if getattr(trip, 'tc_supervisorcost_check', True) else 0.0)
                selling_rto = safe_num(trip.tc_rtocost) if getattr(trip, 'tc_rtocost_check', True) else 0.0
                selling_beta = safe_num(trip.tc_betacost) if getattr(trip, 'tc_betacost_check', True) else 0.0
                selling_cancellation = safe_num(inv.ti_cancellation_charges) if inv else (safe_num(trip.tc_cancellation) if getattr(trip, 'tc_cancellation_check', True) else 0.0)
        ...
        if v_source == 1:
            ...
            # --- Fuel Cost: prorated from Fuelfillinginfo ---
            fuel_expense = round(fuel_lookup.get(trip.id, 0.0), 2)
...
def vendor_bills_pending_mkt_att_report_view(request):
    ...
    if not status_param:
        ...
        if single_status:
            status_param = [s.strip() for s in single_status.split(',') if s.strip()]
...
def vendor_bills_pending_mkt_att_report_ajax_view(request):
    ...
    status_param = request.GET.getlist('status[]') or request.GET.getlist('status')
    if not status_param:
        single_status = request.GET.get('status', '').strip()
        if single_status:
            status_param = [s.strip() for s in single_status.split(',') if s.strip()]
    ...
    selected_status_ids = [int(s) for s in status_param if s.isdigit()]
    if selected_status_ids:
        status_q = Q()
        for sel_status_id in selected_status_ids:
            if sel_status_id == 9:
                # Ready for Invoice or Invoiced
                status_q |= Q(tc_financestatus_id=9) | Q(tc_financestatus__isnull=True, tr_operational_status_id=9) | Q(id__in=invoiced_trip_ids)
            elif sel_status_id == 1:
                # Trip Started (explicit status 1 or no finance/operational status set)
                status_q |= Q(tc_financestatus_id=1) | Q(tc_financestatus__isnull=True, tr_operational_status_id=1) | Q(tc_financestatus__isnull=True, tr_operational_status__isnull=True)
            else:
                status_q |= Q(tc_financestatus_id=sel_status_id) | Q(tc_financestatus__isnull=True, tr_operational_status_id=sel_status_id)
        trip_filters = status_q
    else:
        trip_filters = (
    ...
    # Exclude non-billable Empty trips (allow Business Empty and billable business trips with CNote)
    trip_filters &= Q(tr_consignmentnumber__isnull=False)
    trip_filters &= ~Q(tr_category__category__iexact='Empty') & ~Q(tr_category__category__iexact=' Empty')
...
def transport_mis(request):
    ...
    total_expense_monthly_budget = {i: 0 for i in display_months_indices}
    total_expense_monthly_actual = {i: 0 for i in display_months_indices}

    expense_summaries = [
        op_fixed_summary,
        op_variable_summary,
        non_op_fixed_summary,
        non_op_variable_summary,
        employee_benefits_summary,
        dept_expenses_summary,
        interest_summary
    ]

    for month in display_months_indices:
        total_expense_monthly_budget[month] = sum(
            summary[-1]["monthly_budgets"][month] for summary in expense_summaries
        )
        total_expense_monthly_actual[month] = sum(
            summary[-1]["monthly_expenses"][month] for summary in expense_summaries
        )

    total_expense_monthly_variance = {
        month: total_expense_monthly_budget[month] - total_expense_monthly_actual[month]
        for month in display_months_indices
    }

    total_exp_budget = sum(total_expense_monthly_budget.values())
    total_exp_actual = sum(total_expense_monthly_actual.values())
    total_exp_variance = total_exp_budget - total_exp_actual

    total_expense_summary = {
        "monthly_budget": total_expense_monthly_budget,
        "monthly_actual": total_expense_monthly_actual,
        "monthly_variance": total_expense_monthly_variance,
        "total_budget": total_exp_budget,
        "total_actual": total_exp_actual,
        "total_variance": total_exp_variance,
    }
    ...
    for month in display_months_indices:
        net_monthly_budget[month] = income_summary[-1]["monthly_budgets"][month] - total_expense_monthly_budget[month]
        net_monthly_actual[month] = income_summary[-1]["monthly_expenses"][month] - total_expense_monthly_actual[month]
    ...
    return render(request, "asset_mgt_app/trans_mis.html", {
        ...
        "total_expense_summary": total_expense_summary,
