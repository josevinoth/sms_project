...
@login_required(login_url='login_page')
def invoice_pending_report_excel(request):
    from ..models import TransInvoiceInfo
    ...
    trips = TripdetailInfo.objects.filter(tc_financestatus_id=7).exclude(id__in=invoiced_trip_ids).select_related(
        'tr_enquirynumber', 'tr_enquirynumber__en_customername', 'tr_enquirynumber__en_customerdepartment',
        'tr_consignmentnumber', 'tr_vehicletype', 'tr_vehiclesource', 'tr_departedlocation', 'tr_reportedlocation'
    )
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Invoice Pending Report"
    ...
    # Headers
    for col_num, header in enumerate(INVOICE_PENDING_HEADERS, 1):
    ...
    for idx, trip in enumerate(trips, 1):
        ...
        row = [
            idx, display_date, branch, safe_str(trip.tr_enquirynumber.en_customername),
            cons_no,
            trip.tr_consignmentnumber.co_consignmentdate.strftime("%d-%m-%Y") if trip.tr_consignmentnumber and trip.tr_consignmentnumber.co_consignmentdate else "",
            safe_str(trip.tr_enquirynumber.en_customerdepartment), safe_str(trip.tr_tripnumber),
            safe_str(trip.tr_vehiclenumber), safe_str(trip.tr_vehicletype), safe_str(trip.tr_vehiclesource),
            trip.tr_departeddate.strftime("%d-%m-%Y %H:%M") if trip.tr_departeddate else "",
            trip.tr_updated_at.strftime("%d-%m-%Y") if trip.tr_updated_at else "",
            safe_str(trip.tr_customerref), billing_amount, "Invoice Pending", safe_str(trip.tr_remarks)
        ]
    ...
    filename = f"Invoice_Pending_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
