...
def loadingbay_add_mbl(request, loadingbay_id=0):
    ...
    Currencytype = Currency_type.objects.all()
    Gstexcemption = GstexcemptionInfo.objects.all()
    Sealedoropened = SealedoropenedInfo.objects.all()
    ...
    if request.method == "GET":
        ...
        return render(request, "asset_mgt_app/warehouse_loadingbay_mbl_add.html", context)
    else:
        if loadingbay_id == 0:
...
@login_required(login_url='login_page')
def load_currency_value_mbl(request):
