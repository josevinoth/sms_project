...
@transaction.atomic
@login_required(login_url='login_page')
def goods_mbl_add(request, goods_id=0):
    first_name = request.session.get('first_name')
    ...
    Fumigationaction = Fumigation_ActionInfo.objects.all()
    Damage = DamageInfo.objects.all()
    Location = Location_info.objects.all()
    Unit = UnitInfo.objects.all()
    Bay = BayInfo.objects.all()
    Stacking = StackingInfo.objects.all()
    Checkinout = Check_in_out.objects.all()
    ...
    if request.method == "GET":
        if goods_id == 0:
            ...
            context = {
                ...
                'Fumigationaction': Fumigationaction,
                'Damage': Damage,
                'Location': Location,
                'Unit': Unit,
                'Bay': Bay,
        ...
        return render(request, "asset_mgt_app/goods_mbl_add.html", context)
    else:
        if goods_id == 0:
...
# Delete goods
@login_required(login_url='login_page')
def goods_mbl_delete(request, goods_id):
    goodsinfo = Warehouse_goods_info.objects.get(pk=goods_id)
    goodsinfo.delete()
    return redirect('/SMS/goods_insert')

@login_required(login_url='login_page')
def goods_mbl_update(request):
