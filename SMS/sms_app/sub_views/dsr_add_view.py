...
def dsr_send_email_view(request, pre_gatein_id=None, customer_name=None, subject=None):
    ...
    if request.method == 'POST':
        ...
        if "DHL" in customer_name_str:
            ...
            headers = ["S:NO","IN DATE","Truck reporting at Warehouse (Time)", "Dock Intime at unloading bay",
                       "Dock Outime from unloading bay", "TRUCK NUM", "SHIPPER NAME", "Invoice Number", "PO NO","SO NUMBER","DESTINATION","Total No of Cartons"
                       "VALUE IN", "Invoice Number", "INVOICE WEIGHT","QTY AS PER INV","CBM",
                       "INVOICE WEIGHT", "GROSS WEIGHT", "L CMS",
                       "B CMS", "H CMS", "Carton in DIM", "Volume weight", "LOCATION","Carton No.","DOCS RECEIVED WITH GOODS",
                       "CARGO CONDITION","OTL NO",
                       "Job Number", "Stock Number", "REMARKS"]
        elif "DSV" in customer_name_str:
            headers = ["S:NO","DATE","Truck reporting at Warehouse (Time)","Dock Intime at unloading bay","Dock Outime from unloading bay","TRUCK NUM","SHIPPER NAME","CONSIGNEE","DESTINATION",
                "VALUE IN","Invoice Number","INVOICE WEIGHT","PO NUMBER/BATCH NO","Total No of Cartons",
                "INVOICE WEIGHT(kg)", "ACTUAL WEIGHT(kg)", "UOM", "L CMS",
                "B CMS", "H CMS", "Carton in DIM","Volume weight","LOCATION","Documents received with goods","CARGO CONDITION",
                "Job Number", "Stock Number", "REMARKS"]
        else:
            headers = [
