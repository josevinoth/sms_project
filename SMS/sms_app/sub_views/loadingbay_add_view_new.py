...
from ..models import Loadingbay_Info,WhratemasterInfo,CustomerInfo,Gatein_info,DamagereportInfo,Warehouse_goods_info,Loadingbayimages_Info,Currency_type
from django.core.exceptions import ObjectDoesNotExist
from django.http import HttpResponse
import json

@login_required(login_url='login_page')
def loadingbay_add_new(request, loadingbay_id=0):
    ...
    if request.method == "GET":
        if loadingbay_id == 0:
            ...
            loadingbay_Info_1= Loadingbay_Info.objects.get(lb_job_no=wh_job_id)
            loadingbay_form = LoadingbayddForm(instance=loadingbay_Info_1)
        ...
        return render(request, "asset_mgt_app/loadingbay_add_new.html", context)
    else:
        if loadingbay_id == 0:
            ...
            loadingbay_Info_1 = Loadingbay_Info.objects.get(pk=loadingbay_id)
            loadingbay_form = LoadingbayddForm(request.POST, instance=loadingbay_Info_1)
            loadingbayimg_info=Loadingbayimages_Info.objects.get(lbimg_job_no=wh_job_id)
...
@login_required(login_url='login_page')
def load_currency_value_new(request):
